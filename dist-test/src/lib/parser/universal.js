...
exports.normalisasiWhitespace = normalisasiWhitespace;
exports.pisahkanSaldoAplikasi = pisahkanSaldoAplikasi;
exports.apakahTransaksiPelanggan = apakahTransaksiPelanggan;
exports.parseStrukturAlpines = parseStrukturAlpines;
exports.tebakJenisTransaksiUniversal = tebakJenisTransaksiUniversal;
exports.extractNominalAlpines = extractNominalAlpines;
exports.parseAngkaIndonesia = parseAngkaIndonesia;
exports.extractStatusUniversal = extractStatusUniversal;
exports.extractNamaProdukPaketData = extractNamaProdukPaketData;
exports.apakahNotifikasiPendingAlpines = apakahNotifikasiPendingAlpines;
// ============================================================================
 ...
 */
function normalisasiWhitespace(rawText) {
...
// ============================================================================
const polaSaldoAplikasi = /Saldo\s+(LinkAja|Digipos|Alpines|Dompet)?\s*[\d.,]+(\s*-\s*[\d.,]+\s*=\s*[\d.,]+)?(\s*@\d{1,2}\/\d{1,2}\s+\d{1,2}:\d{2}:\d{2})?/gi;
function pisahkanSaldoAplikasi(rawText) {
    const match = rawText.match(polaSaldoAplikasi);
    if (!match)
        return { teksTanpaSaldo: rawText, saldoInfo: null };
    return {
...
// ============================================================================
const keywordBukanTransaksi = /\b(promo|info|pengumuman|notifikasi rutin|selamat datang|kode referral|yuk\s|cashback|event\s|maintenance|gangguan sistem sedang|pemberitahuan|reminder|newsletter|gebyar|raih hadiah|undian|berhadiah|hadiah\s+(utama|menarik)|program\s+(khusus|spesial)|semarak|meriahkan|kejar target|kerjar target|periode\s+\d|syarat dan ketentuan|s&k berlaku)\b/i;
const keywordTopUpSaldoSendiri = /\b(top\s?up saldo (digipos|alpines|akun|deposit)|isi saldo aplikasi|deposit anda|saldo anda (bertambah|ditambahkan)|pengisian saldo (digipos|alpines|dompet) anda)\b/i;
const keywordStatusTransaksi = /\b(berhasil|sukses|gagal|pending|diproses|akan diproses)\b/i;
function apakahTransaksiPelanggan(rawText) {
    const adaNomorTujuan = /\b(08|62)\d{8,11}\b/.test(rawText);
    const adaReferensi = /\b(SN|REFF|ID Transaksi|IDT|Nomor seri)\b/i.test(rawText);
    const adaStatusTransaksi = keywordStatusTransaksi.test(rawText);
    const adaNominalEksplisit = /\bRp\.?\s?[\d.,]+|\bNOMINAL:\s?[\d.,]+/i.test(rawText);
    ...
    const strukturTransaksiKuat = (adaNomorTujuan || adaReferensi || adaNominalValid) && adaStatusTransaksi;
...
function parseStrukturAlpines(rawText) {
    ...
    let saldoMatch = rawText.match(/Saldo\s+([\d.,-]+)\s*-\s*([\d.,-]+)\s*=\s*([\d.,-]+)\s*@(\d{1,2}\/\d{1,2})\s+(\d{1,2}:\d{2}:\d{2})/i);
    // Fallback ke regex longgar kalau yang ketat tidak match
    if (!saldoMatch) {
        saldoMatch = rawText.match(/Saldo\s+([\d.,-]+)[\s\-]+([\d.,-]+)[\s=]+([\d.,-]+)\s*@/i);
    }
    return {
        headerSegment,
        statusKeyword,
...
async function tebakJenisTransaksiUniversal(rawText, headerSegment) {
    // 1. Cek terhadap kategori resmi yang sudah di-hardcode
    const hasilKnown = await (0, detectJenisTransaksi_1.scoringKeywordKategori)(rawText);
    ...
    const supabase = (0, server_1.createServiceRoleClient)();
...
// ============================================================================
function extractNominalAlpines(_text, headerSegment, snRefSegment, saldoMatch) {
    ...
    if (saldoMatch && saldoMatch[2]) {
        ...
        if (val > 0)
            return { nominal: val, dariSaldoFallback: true };
    }
    ...
    if (nominalLabelMatch) {
        ...
        if (val > 0)
            return { nominal: val, dariSaldoFallback: false };
    }
 ...
 */
function parseSaldoWithValidationFromMatch(saldoMatch) {
    if (!saldoMatch || saldoMatch.length < 4)
        return null;
...
// ============================================================================
const keywordGagal = /\b(gagal|failed|ditolak|bermasalah|gangguan|error|tidak dapat diproses|koneksi\s*(terputus|bermasalah|gagal)|timeout|kadaluarsa|expired)\b/i;
const keywordPending = /\b(pending|diproses|menunggu|mohon\s*tunggu|sedang\s*(diproses|berlangsung)|silakan\s*tunggu|akan\s*diproses|tunggu\s*sms\s*notifikasi)\b/i;
const keywordSukses = /\b(berhasil|sukses|success|telah\s*(dilakukan|selesai))\b/i;
function extractStatusUniversal(rawText, _jenisTransaksi, detailTambahan) {
    ...
    const adaReferensiLengkap = detailTambahan.id_transaksi != null ||
...
// ============================================================================
function extractNamaProdukPaketData(text) {
...
// ============================================================================
const keywordAlpinesPending = /\b(akan\s*diproses|tunggu\s*sms\s*notifikasi|mohon\s*tunggu\s*sebentar)\b/i;
function apakahNotifikasiPendingAlpines(rawText, provider) {
