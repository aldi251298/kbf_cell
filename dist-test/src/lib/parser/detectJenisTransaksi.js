...
exports.detectJenisTransaksi = detectJenisTransaksi;
exports.scoringKeywordKategori = scoringKeywordKategori;
 ...
 */
function scoringPlnToken(rawText) {
 ...
 */
function scoringPaketAtauPulsa(rawText) {
    ...
    const adaAngkaSetelahOperator = /\b(telkomsel|byu|axis|tri|indosat|im3|xl|smartfren)\s+\d{3,6}\b/i;
    if (adaAngkaSetelahOperator.test(rawText) &&
        !/\bpaket\b|\bdata\b/i.test(rawText)) {
        return { kategori: "pulsa", skor: 2 };
    }
    return { kategori: "", skor: 0 };
}
function detectJenisTransaksi(text) {
    const lower = text.toLowerCase();
    const scores = {};
    // Count keyword hits per category
    for (const [kategori, keywords] of Object.entries(keywords_1.JENIS_TRANSAKSI_KEYWORDS)) {
        let score = 0;
    ...
    let bestScore = 0;
    for (const kategori of keywords_1.JENIS_TRANSAKSI_PRIORITY) {
    ...
    for (const [kategori, score] of Object.entries(scores)) {
        if (!keywords_1.JENIS_TRANSAKSI_PRIORITY.includes(kategori) && score > bestScore) {
            bestScore = score;
            best = kategori;
        }
    }
...
function scoringKeywordKategori(rawText) {
    const scores = {};
    for (const [kategori, keywords] of Object.entries(keywords_1.JENIS_TRANSAKSI_KEYWORDS)) {
        let score = 0;
        for (const kw of keywords) {
