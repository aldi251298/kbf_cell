...
import { Loader2, CheckCircle2, AlertCircle, X, Building2, CreditCard, Receipt, Smartphone, Zap, Wallet, Package, HelpCircle } from "lucide-react";
...
const konterOptions = DAFTAR_KONTER.map((k) => ({ value: k.id, label: k.nama }));
...
const transactionIcons: Record<string, React.ComponentType<{ className?: string }>> = {
  pulsa: Smartphone,
  data: Zap,
  pln: Zap,
  emoney: Wallet,
...
export default function FormTransaksiManual() {
  ...
  const [statusSubmit, setStatusSubmit] = useState<"idle" | "loading" | "sukses" | "error">("idle");
  const [errorMessage, setErrorMessage] = useState<string>("");
  const [showNotification, setShowNotification] = useState(false);

  const config = FORM_CONFIG_TRANSAKSI_MANUAL.find((c) => c.jenisTransaksi === jenisTerpilih);
  ...
  const TransactionIcon = config ? transactionIcons[config.jenisTransaksi] || transactionIcons.default : transactionIcons.default;
  ...
  function validasiLengkap(): boolean {
    ...
    return config.fields.filter((f) => f.wajib).every((f) => nilaiField[f.key]?.trim());
  }
      ...
      <div className="relative max-w-2xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-6 fade-in-up">
          ...
          <p className="mt-2 text-base text-gray-600 max-w-xl mx-auto">
            Catat transaksi yang tidak tertangkap notifikasi otomatis dengan cepat dan akurat
          </p>
        </div>

        {/* Notification Popup - Centered Modal */}
        {showNotification && createPortal(
          <div className="fixed inset-0 z-50 flex items-center justify-center animate-fade-in" role="alert" aria-live="polite" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh' }}>
            {/* Backdrop */}
            <div className="absolute inset-0 bg-black/30 backdrop-blur-sm" onClick={() => setShowNotification(false)} />
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className={cn(
                    ...
                    statusSubmit === "sukses" ? "text-green-800" : "text-red-800"
                  )}>
                    {statusSubmit === "sukses" ? "Berhasil" : "Gagal"}
                  </p>
                  <p className={cn(
                    "mt-1 text-sm",
                    statusSubmit === "sukses" ? "text-green-700" : "text-red-700"
                  )}>
                    {statusSubmit === "sukses"
                ...
                <button
                  onClick={() => { setShowNotification(false); setStatusSubmit("idle"); setErrorMessage(""); }}
        ...
        <Card className="border-gray-200 shadow-xl shadow-gray-900/5 overflow-hidden">
          <CardHeader className="px-8 py-6 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-lg font-semibold text-gray-900">Detail Transaksi</CardTitle>
                <p className="text-sm text-gray-500">Isi informasi transaksi di bawah ini</p>
              </div>
            </div>
          </CardHeader>
          <CardContent className="p-8 lg:p-10 space-y-7">
              ...
              <div className="space-y-3 fade-in">
                ...
                <div className="relative">
                  <SelectFloating
                    ...
                    value={jenisTerpilih}
                    onChange={(v) => { setJenisTerpilih(v); setNilaiField({}); }}
            ...
            {konterTerpilih && config && (
              <div className="space-y-6 fade-in border-t border-gray-100 pt-6">
                <div className="flex items-center gap-2 text-sm text-gray-500">
                  ...
                  <span className="font-medium text-gray-700">{config.labelTampilan}</span>
                </div>
                <div className="space-y-5" role="group" aria-labelledby="dynamic-fields-heading">
                  <h3 id="dynamic-fields-heading" className="sr-only">Field Transaksi</h3>
            ...
            {!konterTerpilih && (
              <div className="text-center py-12">
                ...
                <p className="text-gray-500 text-sm">Silakan pilih konter terlebih dahulu untuk memulai</p>
              </div>
            )}
          </CardContent>
        </Card>
    ...
    case "dropdown":
      return (
        <div className="space-y-3">
          ...
          <Select
            options={field.dropdownOptions!.map((opt) => ({ value: opt.value, label: opt.label }))}
            value={nilai ?? ""}
            onChange={onChange}
